...
bisector hyperbolas with the origin, and constructs the cell in their
arrangement that contains the origin. Outputs a single-page PDF.

**Output:** `output/bisector_cell.pdf`

**Note:** The bisector of a line and a point is a parabola, not a
hyperbola. This example uses `Hyperbola(line, origin)` as an
approximation. See `TODO_bisector_cell.md` for details.
...
- **Mode 2** (powers of 2): Compares N = 2^3 … 2^12 with calibrated
  styling (ink area ≤ 25%).
- **Mode 3** (default, 8 pages): Final diagram, prefix diagram, text
  pages with LaTeX, and area-threshold highlighting.
