/*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*
 * mvbb.hpp -
 *     Implmeents an algorithm for computing a diameter,
 *
 * Copyright 2001 Sariel Har-Peled (ssaarriieell@cs.uiuc.edu)
 *
 * * the GNU General Public License as published by the Free
 *   Software Foundation; either version 2, or (at your option)
 *   any later version.
 *
 * or
 *
 * * the GNU Lesser General Public License as published by the Free
 *   Software Foundation; either version 2.1, or (at your option)
 *   any later version.
 *
 *
 * Code is based on the paper:
 *   A Practical Approach for Computing the Diameter of a Point-Set.
 *   Sariel Har-Peled (http://www.uiuc.edu/~sariel)
\*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/

#ifndef  __MVBB__H
#define  __MVBB__H

namespace MVBB { 
#include  <vector>

/* for g++ to quell warnings */
#if (defined(__GNUC__) && (__GNUC__ > 4 || (__GNUC__ == 4 && __GNUC_MINOR__ >= 6)) && !defined(__clang__))
#  pragma GCC diagnostic push /* start new diagnostic pragma */
#  pragma GCC diagnostic ignored "-Wfloat-equal"
#elif (defined(__clang__) && (__clang_major__ > 2 || (__clang_major__ == 2 && __clang_minor__ >= 8)))
#  pragma clang diagnostic push /* start new diagnostic pragma */
#  pragma clang diagnostic ignored "-Wfloat-equal"
#endif

#ifndef MVBB_EXPORT
#  if defined(MVBB_DLL_EXPORTS) && defined(MVBB_DLL_IMPORTS)
#    error "Only MVBB_DLL_EXPORTS or MVBB_DLL_IMPORTS can be defined, not both."
#  elif defined(MVBB_DLL_EXPORTS)
#    define MVBB_EXPORT __declspec(dllexport)
#  elif defined(MVBB_DLL_IMPORTS)
#    define MVBB_EXPORT __declspec(dllimport)
#  else
#    define MVBB_EXPORT
#  endif
#endif

#define MVBB_FLOAT_ZERO(val) (((val) > -1.0e-37) && ((val) < 1.0e-37))

#define  MVBB_DIM   3
typedef  double  real;
typedef  MVBB::real    point_t[ MVBB_DIM ];
typedef  MVBB::real    point_2d_t[ 2 ];
typedef  MVBB::real  * point_2d;
typedef  MVBB::real  * point;
typedef  const MVBB::real  * point_const;

#ifndef __MINMAX_DEFINED
#define __MINMAX_DEFINED

#include<algorithm>
#include<assert.h>
#define min std::min
#define max std::max

#endif /* MIN_MAX */

template <class T>
inline void     exchange( T   & a, T    & b )
{
    T   tmp = a;

    a = b;
    b = tmp;
}

inline MVBB::real   norm( const MVBB::point  pnt ) 
{
    return  sqrt( pnt[ 0 ] * pnt[ 0 ] + pnt[ 1 ] * pnt[ 1 ]
                  + pnt[ 2 ] * pnt[ 2 ] );
}

inline void  pnt_normalize( MVBB::point  pnt )
{
    MVBB::real  len = norm( pnt );
    if  ( MVBB_FLOAT_ZERO(len) )
        return;

    pnt[ 0 ] /= len;
    pnt[ 1 ] /= len;
    pnt[ 2 ] /= len;
}

inline    void  pnt_copy( MVBB::point_t   dest,
                    MVBB::point_t   src )
    {
        dest[ 0 ] = src[ 0 ];
        dest[ 1 ] = src[ 1 ];
        dest[ 2 ] = src[ 2 ];
    }
inline void  pnt_zero( MVBB::point  dst ) {
    dst[ 0 ] = dst[ 1 ] = dst[ 2 ] = 0;
}
inline void  pnt_dump( MVBB::point_const  pnt ) {
    printf( "(%g, %g, %g)\n", pnt[ 0 ], pnt[ 1 ], pnt[ 2 ] );
}


inline MVBB::real  pnt_dot_prod( MVBB::point_const  a,
                                 MVBB::point_const  b )
{
    return  a[ 0 ] * b[ 0 ]
        + a[ 1 ] * b[ 1 ]
        + a[ 2 ] * b[ 2 ];
}

inline void   pnt_cross_prod( const MVBB::point   a, 
                              const MVBB::point   b,
                              const MVBB::point   out )
{    
    out[ 0 ] = a[ 1 ] * b[ 2 ] - a[ 2 ] * b[ 1 ];
    out[ 1 ] = - ( a[ 0 ] * b[ 2 ] - a[ 2 ] * b[ 0 ] );
    out[ 2 ] = a[ 0 ] * b[ 1 ] - a[ 1 ] * b[ 0 ];
}


inline MVBB::real   pnt_distance_2d( MVBB::point_2d  p, 
                                     MVBB::point_2d  q )
{
    MVBB::real  valx = (p[ 0 ] - q[ 0 ]);
    MVBB::real  valy = (p[ 1 ] - q[ 1 ]);

    return  sqrt( valx * valx + valy * valy );
}


inline MVBB::real   pnt_distance( MVBB::point  p, MVBB::point  q )
{
    MVBB::real  valx = (p[ 0 ] - q[ 0 ]);
    MVBB::real  valy = (p[ 1 ] - q[ 1 ]);
    MVBB::real  valz = (p[ 2 ] - q[ 2 ]);

    return  sqrt( valx * valx + valy * valy + valz * valz );
}

// Compute the distance between the projection of p and q into the
// plane orthogonal to the vector dir.
inline MVBB::real   pnt_distance_subspace( MVBB::point  p, MVBB::point  q,
                                  MVBB::point_const  dir )
{
    MVBB::real  valx = (p[ 0 ] - q[ 0 ]);
    MVBB::real  valy = (p[ 1 ] - q[ 1 ]);
    MVBB::real  valz = (p[ 2 ] - q[ 2 ]);

    MVBB::real  len, proj_len;

    len = sqrt( valx * valx + valy * valy + valz * valz );

    proj_len = dir[ 0 ] * valx + dir[ 1 ] * valy + dir[ 2 ] * valz;

    return  sqrt( len * len - proj_len * proj_len );
}

inline void  pnt_init( MVBB::point  pnt, 
                       MVBB::real  x,
                       MVBB::real  y,
                       MVBB::real  z )
{
    pnt[ 0 ] = x;
    pnt[ 1 ] = y;
    pnt[ 2 ] = z;
}


inline void  pnt_init_normalize( MVBB::point  pnt, 
                                 MVBB::real  x,
                                 MVBB::real  y,
                                 MVBB::real  z )
{
    pnt[ 0 ] = x;
    pnt[ 1 ] = y;
    pnt[ 2 ] = z;

    pnt_normalize( pnt );
}

inline bool  pnt_isEqual( const MVBB::point  p,
                          const MVBB::point  q ) 
{
    // Assuming here the MVBB_DIM == 3 !!!!
    return  ( ( MVBB_FLOAT_ZERO(p[0] - q[0]) )
              &&  ( MVBB_FLOAT_ZERO(p[0] - q[0]) )
              &&  ( MVBB_FLOAT_ZERO(p[0] - q[0]) ) );
}

inline void  pnt_scale_and_add( MVBB::point  dest,
                         MVBB::real  coef,
                         MVBB::point_const   vec ) {
    dest[ 0 ] += coef * vec[ 0 ];
    dest[ 1 ] += coef * vec[ 1 ];
    dest[ 2 ] += coef * vec[ 2 ];
}

class  GPointPair
{
public:
    MVBB::real  distance;
    MVBB::point  p, q;

    GPointPair() : p(), q() {
        distance = 0.0;
    }

    void  init( const MVBB::point  _p, 
                const MVBB::point  _q ) {
        p = _p;
        q = _q;
        distance = pnt_distance( p, q );
    }

    void  init( const MVBB::point  _p, 
                const MVBB::point  _q,
                const MVBB::point  proj ) {
        p = _p;
        q = _q;
        distance = pnt_distance_subspace( p, q, proj );
    }

    void  init( const MVBB::point  pnt ) {
        distance = 0;
        p = q = pnt;
    }

    void  update_diam_simple( const MVBB::point  _p, const MVBB::point  _q ) {
        MVBB::real  new_dist;

        new_dist = pnt_distance( _p, _q );
        if  ( new_dist <= distance )
            return;
        
        //printf( "new_dist: %g\n", new_dist );
        distance = new_dist;
        p = _p;
        q = _q;
    }

    void  update_diam( const MVBB::point  _p, const MVBB::point  _q ) { 
        //update_diam_simple( p, _p );
        //update_diam_simple( p, _q );
        //update_diam_simple( q, _p );
        //update_diam_simple( q, _q );
        update_diam_simple( _p, _q );
    }
    void  update_diam_subspace( const MVBB::point  _p, const MVBB::point  _q,
                               const MVBB::point  dir ) {
        MVBB::real  new_dist;

        new_dist = pnt_distance_subspace( _p, _q, dir );
        if  ( new_dist <= distance )
            return;
        
        distance = new_dist;
        p = _p;
        q = _q;
    }

    void  update_diam( GPointPair  & pp ) {
        //update_diam_simple( p, pp.p );
        //update_diam_simple( p, pp.q );
        //update_diam_simple( q, pp.p );
        //update_diam_simple( q, pp.q );
        update_diam_simple( pp.p, pp.q );
    }
};




class   AABBox {
private:
    MVBB::real  min_coords[ MVBB_DIM ];
    MVBB::real  max_coords[ MVBB_DIM ];

public:
    void  init( const AABBox   & a,
                const AABBox   & b ) {
        for  (int  ind = 0; ind < MVBB_DIM; ind++ ) { 
            min_coords[ ind ] = min( a.min_coords[ ind ],
                                     b.min_coords[ ind ] );
            max_coords[ ind ] = max( a.max_coords[ ind ],
                                     b.max_coords[ ind ] );
        }
    }
    void  dump() const {
        MVBB::real  prod, diff;

        prod = 1.0;
        printf( "__________________________________________\n" );
        for  (int  ind = 0; ind < MVBB_DIM; ind++ ) { 
            printf( "%d: [%g...%g]\n", ind, min_coords[ ind ],
                    max_coords[ ind ] );
            diff = max_coords[ ind ] - min_coords[ ind ];
            prod *= diff;
        }
        printf( "volume = %g\n", prod );
        printf( "\\__________________________________________\n" );
    }

    void  center( MVBB::point  out ) const {
        for  (int  ind = 0; ind < MVBB_DIM; ind++ ) {
            out[ ind ] = ( min_coords[ ind ] + max_coords[ ind ] ) / 2.0;
        }
    }

    MVBB::real  volume() const {
        MVBB::real  prod, val;

        prod = 1;        
        for  ( int  ind = 0; ind < MVBB_DIM; ind++ ) {
            val = length_dim( ind );
            prod *= val;
        }

        return  prod;
    }
    void  init() { 
        for  (int  ind = 0; ind < MVBB_DIM; ind++ ) {
            min_coords[ ind ] = 1e20;
            max_coords[ ind ] = -1e20;
        }
    }
    /*
    void  dump() const {
        printf( "---(" );
        for  (int  ind = 0; ind < MVBB_DIM; ind++ ) {
            printf( "[%g, %g] ",
                    min_coords[ ind ],
                    max_coords[ ind ] );
        }
        printf( ")\n" );
        }*/

    const MVBB::real  & min_coord( int  coord ) const {
        return  min_coords[ coord ];
    }
    const MVBB::real  & max_coord( int  coord ) const {
        return  max_coords[ coord ];
    }
    
    void  bound( const MVBB::point   pnt ) {
        //cout << "bounding: " << pnt << "\n";
        for  ( int  ind = 0; ind < MVBB_DIM; ind++ ) {
            if  ( pnt[ ind ] < min_coords[ ind ] )
                min_coords[ ind ] = pnt[ ind ];
            if  ( pnt[ ind ] > max_coords[ ind ] )
                max_coords[ ind ] = pnt[ ind ];
        }
    }

    MVBB::real  length_dim( int  dim )  const {
        return  max_coords[ dim ] - min_coords[ dim ];
    }
    int  getLongestDim() const {
        int  dim = 0;
        MVBB::real  len = length_dim( 0 );

        for  ( int  ind = 1; ind < MVBB_DIM; ind++ ) 
            if  ( length_dim( ind ) > len ) {
                len = length_dim( ind );
                dim = ind;
            }

        return  dim;
    }
    MVBB::real  getLongestEdge() const {
        return  length_dim( getLongestDim() );
    }

    MVBB::real   get_diam() const
    {
        MVBB::real  sum, val;

        sum = 0;        
        for  ( int  ind = 0; ind < MVBB_DIM; ind++ ) {
            val = length_dim( ind );
            sum += val * val;
        }

        return  sqrt( sum );
    }

    // in the following we assume that the length of dir is ONE!!!
    // Note that the following is an overestaime - the diameter of
    // projection of a cube, is bounded by the length of projections
    // of its edges....
    MVBB::real   get_diam_proj( MVBB::point  dir ) const
    {
        MVBB::real  sum, coord;
        MVBB::real  prod, val;
        
        //printf( "get_diam_proj: " );
        sum = 0;
        for  ( int  ind = 0; ind < MVBB_DIM; ind++ ) {
            coord = length_dim( ind );
            //printf( "coord[%d]: %g\n",ind, coord );
            prod = coord * dir [ ind ];
            val = coord * coord - prod * prod;
            assert( val >= 0 );
            sum += sqrt( val );
        }
        //printf( "sum: %g, %g\n", sum, get_diam() );
        
        // sum = squard diameter of the bounding box
        // prod = length of projection of the diameter of cube on the
        //      direction.
        
        return  get_diam();
    }

    MVBB::real  get_min_coord( int  dim ) const {
        return  min_coords[ dim ];
    }
    MVBB::real  get_max_coord( int  dim ) const {
        return  max_coords[ dim ];
    }
};


// BBox is the famous - arbitrary orieted bounding box
class  BBox
{
private:
    MVBB::point_t  dir_1, dir_2, dir_3;
    
    MVBB::real  low_1, high_1;
    MVBB::real  low_2, high_2;
    MVBB::real  low_3, high_3;
    bool  f_init;

public:
    BBox() { 
        f_init = false;
    }

    MVBB::real  volume() const {
        MVBB::real  len1, len2, len3;

        len1 = ( high_1 - low_1 );
        len2 = ( high_2 - low_2 );
        len3 = ( high_3 - low_3 );

        return  len1 * len2 * len3;
    }


    void  set_third_dim_longest() {
        MVBB::point_t  pnt_tmp;

        if  ( ( high_1 - low_1 ) > ( high_3 - low_3 ) ) {
          MVBB::exchange( high_1, high_3 );
          MVBB::exchange( low_1, low_3 );

            memcpy( pnt_tmp, dir_1, sizeof( dir_1 ) );
            //pnt_tmp = dir_1;
            memcpy( dir_1, dir_3, sizeof( dir_3) );
            //dir_1 = dir_3;
            memcpy( dir_3, pnt_tmp, sizeof( dir_3 ) );
            //dir_3 = pnt_tmp;
        }
        if  ( ( high_2 - low_2 ) > ( high_3 - low_3 ) ) {
          MVBB::exchange( high_2, high_3 );
          MVBB::exchange( low_2, low_3 );

            pnt_copy( pnt_tmp, dir_2 );
            pnt_copy( dir_2, dir_3 );
            pnt_copy( dir_3, pnt_tmp );
        }
    }

    MVBB::point  get_dir( int  ind ) {
        if  ( ind == 0 )
            return  dir_1;
        if  ( ind == 1 )
            return  dir_2;
        if  ( ind == 2 )
            return  dir_3;
     
        assert( false );

        return  NULL;
    }
    
    void  combine( MVBB::point  out,
                   double  a_coef, double  b_coef,
                   double  c_coef ) {
        for  ( int  ind = 0; ind < MVBB_DIM; ind++ ) 
            out[ ind ] = a_coef * dir_1[ ind ] 
                + b_coef * dir_2[ ind ] 
                + c_coef * dir_3[ ind ];
    }

    void  get_vertex( double sel1, double sel2, double sel3, double *p_x, double *p_y, double *p_z ) {
        MVBB::real  coef1, coef2, coef3;
        MVBB::point_t  pnt;

        coef1 = low_1 + sel1 * ( high_1 - low_1 );
        coef2 = low_2 + sel2 * ( high_2 - low_2 );
        coef3 = low_3 + sel3 * ( high_3 - low_3 );

        pnt_zero( pnt );
        pnt_scale_and_add( pnt, coef1, dir_1 );
        pnt_scale_and_add( pnt, coef2, dir_2 );
        pnt_scale_and_add( pnt, coef3, dir_3 );

	(*p_x) = pnt[0];
	(*p_y) = pnt[1];
	(*p_z) = pnt[2];

    }


    void  dump_vertex( double  sel1, double  sel2, double  sel3 ) const {
        MVBB::real  coef1, coef2, coef3;

        //printf( "selection: (%g, %g, %g)\n", sel1, sel2, sel3 );
        coef1 = low_1 + sel1 * ( high_1 - low_1 );
        coef2 = low_2 + sel2 * ( high_2 - low_2 );
        coef3 = low_3 + sel3 * ( high_3 - low_3 );

        //printf( "coeficients: (%g, %g,  %g)\n", 
        //        coef1, coef2, coef3 );

        MVBB::point_t  pnt;
        pnt_zero( pnt );
        
        //printf( "starting...\n" );
        //pnt_dump( pnt );
        pnt_scale_and_add( pnt, coef1, dir_1 );
        //pnt_dump( pnt );
        pnt_scale_and_add( pnt, coef2, dir_2 );
        //pnt_dump( pnt );
        pnt_scale_and_add( pnt, coef3, dir_3 );
        //pnt_dump( pnt );

        pnt_dump( pnt );
    }

    void  dump() const {
        printf( "-----------------------------------------------\n" );
        dump_vertex( 0, 0, 0 );
        dump_vertex( 0, 0, 1 );
        dump_vertex( 0, 1, 0 );
        dump_vertex( 0, 1, 1 );
        dump_vertex( 1, 0, 0 );
        dump_vertex( 1, 0, 1 );
        dump_vertex( 1, 1, 0 );
        dump_vertex( 1, 1, 1 );
        printf( "volume: %g\n", volume() );
        printf( "Directions:\n" );
        pnt_dump( dir_1 );
        pnt_dump( dir_2 );
        pnt_dump( dir_3 );
        printf( "prods: %g, %g, %g\n",
                pnt_dot_prod( dir_1, dir_2 ),
                pnt_dot_prod( dir_1, dir_3 ),
                pnt_dot_prod( dir_2, dir_3 ) );

        printf( "--------------------------------------------------\n" );
        //printf( "range_1: %g... %g\n", low_1, high_1 );
        //printf( "range_2: %g... %g\n", low_2, high_2 );
        //printf( "range_3: %g... %g\n", low_3, high_3 );
        //printf( "prd: %g\n", pnt_dot_prod( dir_1, dir_2 ) );
        //printf( "prd: %g\n", pnt_dot_prod( dir_2, dir_3 ) );
        //printf( "prd: %g\n", pnt_dot_prod( dir_1, dir_3 ) );
    }


    void  init( const AABBox  & bb ) {
        pnt_init( dir_1, 1, 0, 0 );
        pnt_init( dir_2, 0, 1, 0 );
        pnt_init( dir_3, 0, 0, 1 );
    
        low_1 = bb.min_coord( 0 );
        high_1 = bb.max_coord( 0 );

        low_2 = bb.min_coord( 1 );
        high_2 = bb.max_coord( 1 );

        low_3 = bb.min_coord( 2 );
        high_3 = bb.max_coord( 2 );
        f_init = true;
    }

    void  init( const MVBB::point  _dir_1,
                const MVBB::point  _dir_2,
                const MVBB::point  _dir_3 ) {
        memset( this, 0, sizeof( BBox ) );

        pnt_copy( dir_1, _dir_1 );
        pnt_copy( dir_2, _dir_2 );
        pnt_copy( dir_3, _dir_3 );
        pnt_normalize( dir_1 );
        pnt_normalize( dir_2 );
        pnt_normalize( dir_3 );

        if  ( ( ! (fabs( pnt_dot_prod( dir_1, dir_2 ) ) < 1e-6 ) )
              ||  ( ! (fabs( pnt_dot_prod( dir_1, dir_3 ) ) < 1e-6 ) )
              ||  ( ! (fabs( pnt_dot_prod( dir_2, dir_3 ) ) < 1e-6 ) ) ) {
            printf( "should be all close to zero: %g, %g, %g\n",
                    pnt_dot_prod( dir_1, dir_2 ),
                    pnt_dot_prod( dir_1, dir_3 ),
                    pnt_dot_prod( dir_2, dir_3 ) );
            pnt_dump( _dir_1 );
            pnt_dump( _dir_2 );
            pnt_dump( _dir_3 );
            fflush( stdout );
            fflush( stderr );
            assert( fabs( pnt_dot_prod( dir_1, dir_2 ) ) < 1e-6 );
            assert( fabs( pnt_dot_prod( dir_1, dir_3 ) ) < 1e-6 );
            assert( fabs( pnt_dot_prod( dir_2, dir_3 ) ) < 1e-6 );
        }

        // The following reduce the error by slightly improve the
        // orthoginality of the third vector. Doing to the second
        // vector is not efficient...
        pnt_scale_and_add( dir_3, -pnt_dot_prod( dir_3, dir_1 ),
                           dir_1 );
        pnt_scale_and_add( dir_3, -pnt_dot_prod( dir_3, dir_2 ),
                           dir_2 );
        pnt_normalize( dir_3 );
        //printf( "__should be all close to zero: %g, %g, %g\n",
        //         pnt_dot_prod( dir_1, dir_2 ),
        //        pnt_dot_prod( dir_1, dir_3 ),
        //        pnt_dot_prod( dir_2, dir_3 ) );        
    }

    void  bound( const MVBB::point  pnt ) {
        MVBB::real  prod_1, prod_2, prod_3;

        prod_1 = pnt_dot_prod( dir_1, pnt );
        prod_2 = pnt_dot_prod( dir_2, pnt );
        prod_3 = pnt_dot_prod( dir_3, pnt );

        if  ( ! f_init ) {
            f_init = true;
            low_1 = high_1 = prod_1;
            low_2 = high_2 = prod_2;
            low_3 = high_3 = prod_3;
            return;
        }
        if  ( prod_1 < low_1 )
            low_1 = prod_1;
        if  ( prod_2 < low_2 )
            low_2 = prod_2;
        if  ( prod_3 < low_3 )
            low_3 = prod_3;

        if  ( prod_1 > high_1 )
            high_1 = prod_1;
        if  ( prod_2 > high_2 )
            high_2 = prod_2;
        if  ( prod_3 > high_3 )
            high_3 = prod_3;
    }

    // compute the coordinates as if the bounding box is unit cube of
    // the grid
    void  get_normalized_coordinates( MVBB::point  in,
                                      MVBB::point  out ) {
        out[ 0 ] = ( pnt_dot_prod( dir_1, in ) - low_1 ) / ( high_1 - low_1 );
        out[ 1 ] = ( pnt_dot_prod( dir_2, in ) - low_2 ) / ( high_2 - low_2 );
        out[ 2 ] = ( pnt_dot_prod( dir_3, in ) - low_3 ) / ( high_3 - low_3 );
    }

};

MVBB_EXPORT GPointPair   approx_diam( MVBB::point  * start, int  size,
                                      MVBB::real  eps );
MVBB_EXPORT GPointPair   approx_diam( MVBB::point  * start, int  size,
                                MVBB::real  eps );
MVBB_EXPORT MVBB::real   approx_diam( MVBB::real  * start, int  size,
                                MVBB::real  eps );
MVBB_EXPORT GPointPair   approx_diam_pair( MVBB::real  * start, int  size,
                                     MVBB::real  eps );
MVBB_EXPORT GPointPair   approx_diam_UDM( MVBB::real  * start, int  size,
                                         MVBB::real  eps );
MVBB_EXPORT BBox         approx_const_mvbb( MVBB::point  * start, int  size,
                                      MVBB::real  eps, 
                                      AABBox  * p_ap_bbox );    
MVBB_EXPORT MVBB::point  * convert( MVBB::real  * start, int  size );
MVBB_EXPORT BBox   approx_mvbb( MVBB::point  * start, int  size,
                                MVBB::real  eps ) ;
MVBB_EXPORT BBox   approx_mvbb_grid( MVBB::point  * start, int  size,
                                     int  grid_size );
MVBB_EXPORT BBox   approx_mvbb_grid_sample( MVBB::point  * start, int  size,
                                            int  grid_size, int  sample_size );
MVBB_EXPORT BBox   approx_mvbb_grid_sample( MVBB::real  * start, int  size,
                                            int  grid_size, int  sample_size );


MVBB_EXPORT void        generate_orthonormal_base( MVBB::point  in,
                                       MVBB::point  out1,
                                       MVBB::point  out2 );

/**************************************************************************************
 * Two dimensional points/convex-hull
\**************************************************************************************/  
class  point2d
{
public:
  MVBB::real  x, y;
  MVBB::point  src;

  point2d( const MVBB::real  & base_x,
          const MVBB::real  & base_y ) {
    src = NULL;
    x = base_x;
    y = base_y;
  }

  void  init( MVBB::point  pnt,
              MVBB::point  base_x,
              MVBB::point  base_y ) {
        src = pnt;
        x = pnt_dot_prod( pnt, base_x );
        y = pnt_dot_prod( pnt, base_y );
    }

    MVBB::real  dist( const point2d  & pnt ) {
        return  sqrt( ( x - pnt.x ) * ( x - pnt.x )
                      + ( y - pnt.y ) * ( y - pnt.y ) );
    }

    void  dump() const {
        printf( "(%g, %g)\n", x, y );
    }
    bool  equal( const point2d  & pnt ) const {
        return  ( ( x == pnt.x )  &&  ( y == pnt.y ) );
    }
    bool  equal_real( const point2d  & pnt ) const {
      MVBB::real x_delta =  fabs( x - pnt.x );
      MVBB::real y_delta =  fabs( y - pnt.y );

      if  ( ( x_delta == 0.0 )  &&  ( y_delta == 0.0 ) )
        return  true;
      MVBB::real  x_sum = fabs( x ) + fabs( pnt.x );
      if  ( x_delta > ( x_sum * 1e-6 ) )
        return  false;
      MVBB::real  y_sum = fabs( y ) + fabs( pnt.y );
      if  ( y_delta > ( y_sum * 1e-6 ) )
        return  false;

      return  true;
    }
};

typedef  point2d  * point2d_ptr;
class  vec_ptr_points2d : public std::vector<point2d_ptr> {};


void   dump( vec_ptr_points2d   & vec );
vec_ptr_points2d  points_2d_to_pointers(std::vector<point2d>& points);
void   convex_hull_2d( vec_ptr_points2d  & pnts, vec_ptr_points2d  & out );
void   verify_convex_hull_2d( vec_ptr_points2d  & in, vec_ptr_points2d  & ch );




#if (defined(__GNUC__) && (__GNUC__ > 4 || (__GNUC__ == 4 && __GNUC_MINOR__ >= 6)) && !defined(__clang__))
#  pragma GCC diagnostic pop /* end ignoring warnings */
#elif (defined(__clang__) && (__clang_major__ > 2 || (__clang_major__ == 2 && __clang_minor__ >= 8)))
#  pragma clang diagnostic pop /* end ignoring warnings */
#endif

};

#else   /* __MVBB__H */
#error  Header file mvbb.h included twice
#endif  /* __MVBB__H */

/* mvbb.h - End of File ------------------------------------------*/
