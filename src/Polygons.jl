...
"""
abstract type AbsPolygon{D,T} <: AbsFMS end
...
"""
struct Polygon{D,T} <: AbsPolygon{D,T}
...
struct MatPolygon{D,T,M<:AbstractMatrix{T},V<:AbstractVector{Point{D,T}}} <: AbsPolygon{D,T}
    ...
    function MatPolygon(mat::M) where {T,M<:AbstractMatrix{T}}
        D = size(mat, 1)
...
Return the dimension of the points in the polygon.
"""
@inline dim(P::AbsPolygon{D,T}) where {D,T} = D

"""
    dist(P::AbsPolygon, i::Int, j::Int)

Distance between vertex `i` and vertex `j` of the polygon.
Part of the `AbsFMS` interface.
"""
@inline function dist(P::AbsPolygon, i::Int, j::Int)
    if i == j
        return 0.0
    end
...
"""
@inline Points(P::AbsPolygon) = P.pnts
...
"""
@inline cardin(P::AbsPolygon) = length(P.pnts)
...
"""
function translate!(P::AbsPolygon{D,T}, v::Point{D,T}) where {D,T}
    P.pnts .+= Ref(v)
    return P
end
...
"""
function scale!(P::AbsPolygon{D,T}, s::Real) where {D,T}
    P.pnts .*= s
    return P
end
...
"""
function geom_length(poly::AbsPolygon{D,T}) where {D,T}
    len = 0.0
    pnts = poly.pnts
...
"""
function prefix_lengths(poly::AbsPolygon{D,T}) where {D,T}
    n = length(poly)
...
"""
function at(poly::AbsPolygon{D,T}, t::Real) where {D,T}
    n = length(poly)
...
Base.length(P::AbsPolygon) = length(P.pnts)
Base.getindex(P::AbsPolygon, i::Int) = P.pnts[i]
Base.setindex!(P::AbsPolygon, v, i::Int) = (P.pnts[i] = v)
Base.firstindex(P::AbsPolygon) = 1
Base.lastindex(P::AbsPolygon) = length(P.pnts)
Base.iterate(P::AbsPolygon) = iterate(P.pnts)
Base.iterate(P::AbsPolygon, state) = iterate(P.pnts, state)
Base.push!(P::Polygon, p::Point) = push!(P.pnts, p)
Base.pop!(P::Polygon) = pop!(P.pnts)
Base.empty!(P::Polygon) = empty!(P.pnts)
Base.first(P::AbsPolygon) = first(P.pnts)
Base.last(P::AbsPolygon) = last(P.pnts)
# For AbsFMS interface, size should return the number of points as an Int
Base.size(P::AbsPolygon) = length(P.pnts)
Base.size(P::AbsPolygon, dim::Int) = dim == 1 ? length(P.pnts) : 1

function Base.show(io::IO, poly::Polygon{D,T}) where {D,T}
    print(io, "Polygon{$D,$T} with $(length(poly)) points")
end

function Base.show(io::IO, poly::MatPolygon{D,T}) where {D,T}
    print(io, "MatPolygon{$D,$T} with $(length(poly)) points")
end
...
"""
function Base.Matrix(P::AbsPolygon{D,T}) where {D,T}
...
"""
function write_plt(filename::String, P::AbsPolygon)
...
"""
function simplify(P::AbsPolygon{D,T}, r::Real) where {D,T}
    n = length(P)
...
"""
function sample_uniformly(P::AbsPolygon{D,T}, n::Int) where {D,T}
...
function rand_polygon(D::Int, T::Type{<:Real}, n::Int)
    return Polygon([rand_point(D, T) for _ = 1:n])
end
...
export Polygon, AbsPolygon, MatPolygon, Polygon2I, Polygon2F, Polygon3F
